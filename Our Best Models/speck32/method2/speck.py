...
def make_train_data_twodiff(diff1,diff2,n, nr, pairs,diff=(0x0040,0)):
    # print("make_train_data_twodiff")
    Y = np.frombuffer(urandom(n), dtype=np.uint8); Y = Y & 1;
    Y1= np.tile(Y,pairs);
    keys = np.frombuffer(urandom(8*n),dtype=np.uint16).reshape(4,-1);
    keys = np.tile(keys,pairs);
    ...
    # plain1l[Y1==0] = np.frombuffer(urandom(2*num_rand_samples),dtype=np.uint16);
    # plain1r[Y1==0] = np.frombuffer(urandom(2*num_rand_samples),dtype=np.uint16);
    plain1l = plain0l ^ diff1[0]; plain1r = plain0r ^ diff1[1];
    # num_rand_samples = np.sum(Y1==0); 
    plain1l[Y1==0] = plain0l[Y1==0] ^ diff2[0]
    plain1r[Y1==0] = plain0r[Y1==0] ^ diff2[1]
